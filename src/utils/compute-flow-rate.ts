...
const newFlowRate = (config: SkytechPowerFlowCardConfig, value: number): number => {
...
const oldFlowRate = (config: SkytechPowerFlowCardConfig, value: number, total: number): number => {
  const min = config?.min_flow_rate!;
  const max = config?.max_flow_rate!;
  return max - (value / (total > 0 ? total : value)) * (max - min);
};

export const computeFlowRate = (config: SkytechPowerFlowCardConfig, value: number, total: number): number => {
